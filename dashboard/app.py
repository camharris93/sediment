"""sediment dashboard — Report (editable, AI-charted) + Ask (NL->SQL chat with a
live L1-L7 trace) + Build (promote chat -> dbt model), over one local DuckDB file.
...
from engine.config import (
    WAREHOUSE_PATH, active_dataset, app_mode, has_anthropic_key, is_build_mode,
)

DATASET = active_dataset()
st.set_page_config(page_title="sediment — AnAge", layout="wide")
...
def marts_exist() -> bool:
    try:
        df = query_df("select table_name from information_schema.tables where table_schema='marts'")
        return {"mart_longevity_by_class", "mart_aging_outliers"} <= set(df["table_name"])
    except Exception:
        return False
...
def render_chart(df: pd.DataFrame, spec) -> bool:
    """Render a ChartSpec over df. Returns True if a chart was drawn."""
    ct = spec.chart_type
    ...
    try:
        if ct == "bar":
            ch = alt.Chart(df).mark_bar().encode(
                x=alt.X(f"{spec.x}:N", sort="-y", title=spec.x),
                y=alt.Y(f"{spec.y}:Q"),
            ...
            enc = dict(x=alt.X(f"{spec.x}:Q", scale=xs, title=spec.x),
                       y=alt.Y(f"{spec.y}:Q", scale=ys, title=spec.y),
                       tooltip=list(df.columns)[:6])
            ...
            ch = alt.Chart(m).mark_bar().encode(
                x=alt.X(f"{spec.x}:N", title=spec.x), y="value:Q",
                color="metric:N", xOffset="metric:N")
        else:
            return False
        if spec.title:
...
# ─────────────────────────────────────────────────────────────────────────────
# Live L1-L7 trace (sql-engine style) — driven by the orchestrator's on_event
# ─────────────────────────────────────────────────────────────────────────────
_LAYER_NAMES = {"L1": "Intent", "L2": "Generation", "L3": "Static valid.",
                "L4": "Dry-run (EXPLAIN)", "L5": "Execution", "L6": "Plausibility",
                "L7": "Translation"}
...
class LiveTrace:
    ...
    def __call__(self, ev) -> None:
        ...
        if ev.kind == "layer_start":
            ...
            self._finish(layer, name, ev.payload, ok=True)
        elif ev.kind == "validation_fail":
            self._finish(layer, name, ev.payload, ok=False)
    ...
    def _finish(self, layer, name, payload, ok) -> None:
        ...
        try:
            if layer == "L1" and ok:
                ...
                if intent.get("restated_question"):
                    h.markdown(f"**Restated:** {intent['restated_question']}")
                if intent.get("target_tables"):
                    h.markdown("**Tables:** " + ", ".join(f"`{t}`" for t in intent["target_tables"]))
                if intent.get("assumptions"):
                    h.markdown("**Assumptions:**")
                ...
                h.markdown(f"{payload.get('row_count',0)} rows · {payload.get('elapsed_ms',0)} ms"
                           + ("  ·  ⚠️ truncated" if payload.get("truncated") else ""))
            elif layer == "L6":
                if ok:
            ...
            pass
        warn = (not ok) and layer in ("L3", "L4", "L6")  # caught-and-handled, not fatal
        state = "error" if (not ok) else "complete"
        label = f"{layer} · {name}"
        icon = "✅" if ok else ("🔧" if warn else "⛔")
        h.update(label=f"{label}   {icon}", state=state, expanded=not ok)
...
with left:
    st.title("🧬 sediment — Animal Ageing & Longevity")
...
if not WAREHOUSE_PATH.exists() or not marts_exist():
    st.warning("Marts aren't built yet. Run **`python run.py up`** at the repo root first.")
    st.stop()
...
with tab_report:
    totals = query_df(
        "select (select count(*) from staging.stg_anage) species, "
        "(select count(*) from marts.mart_longevity_by_class) classes, "
        "(select count(*) from marts.mart_aging_outliers) fit_species, "
        "(select round(max(max_longevity_yrs)) from staging.stg_anage) max_life"
    ).iloc[0]
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Species", f"{int(totals.species):,}")
    c2.metric("Taxonomic classes", f"{int(totals.classes):,}")
    c3.metric("Species in size→lifespan fit", f"{int(totals.fit_species):,}")
    c4.metric("Longest-lived (yrs)", f"{int(totals.max_life):,}")
    st.divider()

    st.subheader("Longevity by taxonomic class")
    by_class = query_df(
        "select class, n_species, n_with_longevity, avg_longevity_yrs, max_longevity_yrs "
        "from marts.mart_longevity_by_class where n_with_longevity >= 5 "
    ...
    scatter = query_df(
        "select common_name, class, adult_weight_g, max_longevity_yrs, "
        "predicted_longevity_yrs, longevity_ratio, overperformer_rank from marts.mart_aging_outliers")
    base = alt.Chart(scatter)
    pts = base.mark_circle(opacity=0.45).encode(
        ...
        tooltip=["common_name", "class", "adult_weight_g", "max_longevity_yrs",
                 "predicted_longevity_yrs", "longevity_ratio"])
    ...
    # ── Custom (AI-built) report blocks — rendered for everyone ──────────────
    ...
    for b in blocks:
        ...
        if is_build_mode():
            ...
            if a.button("▲", key=f"up_{b.id}"):
                move_block(DATASET, b.id, -1); st.rerun()
            if bb.button("▼", key=f"dn_{b.id}"):
                move_block(DATASET, b.id, +1); st.rerun()
            if cc.button("🗑", key=f"del_{b.id}"):
                delete_block(DATASET, b.id); st.rerun()
        with st.expander("SQL"):
            st.code(b.sql, language="sql")

    # ── Customize (build mode only): AI chart builder ───────────────────────
    if is_build_mode():
        st.divider()
        with st.expander("✏️ Customize report — add an AI-built chart", expanded=bool(st.session_state.get("report_seed"))):
            ...
            if src == "A mart":
                ...
                sql = f"select * from marts.{tbl} limit 1000"
            else:
                sql = st.text_area("SQL", value=seed.get("sql", "select * from marts.mart_aging_outliers limit 500"),
                                   height=120, key="rep_sql")
            instr = st.text_input("What do you want to see? (optional)",
                                  value=seed.get("instruction", ""), key="rep_instr")
            ...
            if prev:
                ...
                if not render_chart(df, spec):
                    st.info(f"Chart type `{spec.chart_type}` — showing table. {spec.reasoning or ''}")
                ...
                if st.button("➕ Add to report", type="primary", key="rep_add"):
                    ...
                    add_block(DATASET, title=prev["title"], sql=prev["sql"], spec=spec)
                    st.session_state.pop("rep_preview", None)
                    st.session_state.pop("report_seed", None)
                    st.success("Added. It's now part of the report for everyone.")
                    st.rerun()
...
# ═════════════════════════════════════════════════════════════════════════════
with tab_ask:
    st.caption("Ask in plain English. Watch the L1–L7 pipeline ground, generate, validate, and run "
               "(read-only) — every number traces to the SQL shown.")
    if not has_anthropic_key():
        ...
        for turn in st.session_state.chat:
            ...
            with st.chat_message("assistant"):
                ...
                st.caption(f"Trust: {_BADGE.get(turn['badge'], turn['badge'])} · "
                           f"{turn['attempts']} attempt(s) · {turn['ms']}ms")

        q = st.chat_input("e.g. which mammals live longest for their size?")
        if q:
            ...
            with st.chat_message("assistant"):
                ...
                res = run_to_executed_answer(q, grounding(), on_event=LiveTrace(trace_box))
                fa = res.final_answer
                st.markdown("**Answer**")
                ...
                st.session_state.chat.append({
                    "q": q, "answer": fa.explanation if fa else "", "sql": res.sql,
                    "rows": res.rows, "badge": fa.trust_badge.value if fa else "failed",
                    "attempts": res.attempts, "ms": fa.elapsed_ms if fa else 0, "status": res.status})
                if res.status == "executed" and res.sql:
                    st.session_state.last_answer = st.session_state.chat[-1]
                    if is_build_mode():
                        h1, h2 = st.columns(2)
                        if h1.button("📈 Chart this in the Report"):
                            st.session_state.report_seed = {"sql": res.sql, "instruction": q, "title": q[:60]}
                            st.toast("Seeded the Report → Customize panel. Open the 📊 Report tab.")
                        h2.info("Or open **🛠 Build a model** to turn this into a mart.")
...
with tab_build:
    if not is_build_mode():
        ...
        st.markdown(
            "This is the read-only / shared deployment. Anyone can **Ask** questions, but turning "
    ...
    if not last:
        st.info("Ask a question in the **💬 Ask** tab first. Your latest executed answer shows up here.")
        st.stop()
    ...
    if b2.button("💾 Save as dbt model (review)", use_container_width=True):
        ...
        try:
            path = propose_model(name, sql, question=last["q"], rationale=rationale)
    ...
    if b3.button("🚀 Build into marts (dbt run)", use_container_width=True, type="primary"):
        from engine.modeling import materialize_model
        try:
            with st.spinner("Writing model + dbt run…"):
                r = materialize_model(name, sql, question=last["q"], rationale=rationale)
            if r.ok:
                st.success(f"Built into marts: {Path(r.model_path).name}. It's now a real, tested mart.")
                st.cache_resource.clear()
                from engine.modeling import validate_model_name
                st.session_state.report_seed = {
                    "sql": f"select * from marts.{validate_model_name(name)} limit 1000",
                    "instruction": last["q"], "title": last["q"][:60]}
                st.info("Now open **📊 Report → Customize** to chart it (already seeded), or commit the model.")
